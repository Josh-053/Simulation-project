...
auc_summary %>%
  kable(
    ...
    col.names = c("Regimen", "Median AUC", "Mean AUC", "SD", "PTA (%)"),
    align    = c("l", "r", "r", "r", "r"),
    longtable = TRUE
  )
```

Given that the body weight in this study was added as a covariate for PK parameters (see previous page), it was important to assess the dosing regimen across studied BW range. @tbl-AUC2 shows summary statistics of AUC~0-24h~ of dosing regimen **9.50 mg/kg q24h** for the minimum, median and maximum BW.
...
auc_summary %>%
  kable(
    ...
    booktabs = TRUE,    # nicer horizontal rules
    digits   = 4,       # numeric rounding
    col.names = c("Body Weight (kg)", "Dose (mg)", "Median AUC", "Mean AUC", "SD", "PTA (%)"),
    align    = c("l", "r", "r", "r", "r", "r"),
    longtable = TRUE
  )
```

Multiplying the normalized dose, 9.50 mg/kg, by BW gives different amounts for different BW values, as shown in the table above. The results in the table above show that 9.50 mg/kg elicit optimal exposure across the studied BW range. This can be explained by the positive relationship between BW and volume of distribution (`V`) as demonstrated in the model code. This means that a patient with larger BW has a larger V, and therefore lower plasma concentration. In other words, raising the dose compensates for the reduction in exposure driven by the increase in V.

{{< pagebreak >}} 

Below is @fig-regimens that shows the IPRED plotted against time for regimen of 9.50 mg/kg for minimum, median and maximum body weights. As explained in the previous page, the increase in dose balances out the increase of V so that the exposure remains optimal for all studied patients. The figure also shows that patients with higher body weight clear the drug slower, as seen from the decreasing slope.
...
In this study, a popPK model was used for the simulation of regimens for dose optimizations of triazoconazole in children with Brontogen fibropathy (BF). As seen in @tbl-AUC1, the PTA was zero percent for dosing regimen of **150 mg q24h**. This indicates that the dose does not evoke sufficient exposure in the patients. Based on the PTA, the regimen of **9.50 mg/kg q24h** was regarded as suitable for the studied patients.

There were limitations in the study: due to limited data on response and toxicity profile, the safety of the dosing regimen of triazoconazole could not be assessed. Furthermore, no exposure-response relationship could be established. Once sufficient response and toxicity data are available, the safety of the proposed dosing regimen will be evaluated, and exposure-response analysis will be carried out.

## Conclusion

The study suggests that the initial dosing regimen of **150 mg q24h**, elicits insufficient exposure. Dosing regimen of **9.50 mg/kg q24h** is proposed and expected to achieve the target exposure and elicit optimal response against *S. viridicaulis* in children with Brontogen fibropathy.
...
We would also like to express our gratitude to Emmanuel Niyigena for his assistance and creative mind for coming up with this research.
