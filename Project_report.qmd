...
Multiplying the normalized dose 9.50 mg/kg by BW gives different quantities for different BW values, as shown in the table above. However, their AUC~0-24~ and PTA are comparable, which can be explained by the positive relationship between BW and volume of distribution (`V`) as demonstrated in the model code. This means that a patient with larger BW has a larger V, therefore lower plasma concentration. In other words, raising the dose compensates for the reduction in exposure driven by the increase in V, that in turn is increased by BW.

{{< pagebreak >}} 

Below is @fig-regimens that shows the IPRED plotted against time for regimen of 9.50 mg/kg for minimum, median and maximum body weights. As explained in the previous page, the increase in dose balances out the increase of V so that the AUC~0-24~ remains comparable across the studied BW range. The figure also shows that patients with higher body weight clear the drug slower, as seen from the decreasing slope.
